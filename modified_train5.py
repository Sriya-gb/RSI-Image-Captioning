...
def validate(args, val_loader, encoder, decoder, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            # References
            sort_ind = sort_ind.cpu() if allcaps.device.type == 'cpu' else sort_ind.to(allcaps.device)
...
if __name__ == '__main__':
    ...
    # Data parameters
    parser.add_argument('--data_folder', default="./RSICD_captions/data/",help='folder with data files saved by create_input_files.py.')
    ...
    for encoder_layers, decoder_layers in [(3,3)]: #,,(0,6),(2,2),
        ...
        # Read word map
        word_map_file = os.path.join('/content/Code/MLAT/RSICD_captions/data/', 'WORDMAP_' + args.data_name + '.json')
       ...
       data_folder='/content/Code/MLAT/' + args.data_folder,
       data_name=args.data_name,
       split='TRAIN',
        transform=transforms.Compose([normalize])
)
        subset_size = int(len(full_train_dataset) * 0.01)
        ...
        val_loader = torch.utils.data.DataLoader(
            CaptionDataset('/content/Code/MLAT/'+args.data_folder, args.data_name, 'TEST', transform=transforms.Compose([normalize])),
            batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
